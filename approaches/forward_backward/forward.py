...
columns = ["inputs","questions", "gold answers", "full output", "predictions"] + ["greedy_option"] + [f"op{i+1}" for i in range(args.max_options)] + ["greedy_output"] + [f"out{i+1}" for i in range(args.max_options)] + ["log_probs"]
...
for i,elem in zip([index for index in range(args.start,args.end)],qa[args.start:args.end]):
    ...
    greedy_out = greedy_out.split(f_instring)[-1].strip()
    ...
    iodf.loc[i] = [f_instring,elem['q'],elem['a'],full_output,pred_options] + f_answers + f_outputs + [log_probs]

    if i%50 == 49:
        iodf.to_csv(f"llama2-7b-chat-outputs/forward/medqa_test_{args.uniq_options}-{args.num_options}_sampled_{args.start}-{args.end}.csv", index = False)
        print("RESULT CHECKPOINT SAVED")

iodf.to_csv(f"/dccstor/ojasgr/scripts/approaches/forward_backward/llama2-7b-chat-outputs/forward/medqa_test_{args.uniq_options}-{args.num_options}_sampled_{args.start}-{args.end}.csv", index = False)
print("SAVED IO")
